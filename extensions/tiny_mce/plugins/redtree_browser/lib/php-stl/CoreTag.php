...
class CoreTag extends Tag
{
    ...
    public function _forEach(DOMElement &$element)
    {
        ...
        $varStatus = $this->getUnquotedAttr($element, 'varStatus', '__dindex');        
        ...
        if (preg_match('/,/', $var)) {
            ...
            $this->compiler->write('<?php $__xoxoxox=' . $list . ';$' . $varStatus . '=new LoopTagStatus();$' . $varStatus . '->count=count($__xoxoxox);
                                    for(;$' . $varStatus . '->index<$' . $varStatus . '->count;$' . $varStatus . '->index++){
                                    $' . $varStatus . '->current=$' . $var . '=$__xoxoxox[$' . $varStatus . '->index]; ?>' );
